          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            ...
            <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form" >
              <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
                <BaseInput name="name" type="text" placeholder="Define Variant name" class="outlined" label="Variant Name"/>
                <div class="select-option flex flex-col gap-1 text-left">
                  <label for="vehicleId" class="text-xs font-bold">GPS Manufacturer</label>
                  <select name="manufacturerId" v-model="manufacturerId" class="cursor-pointer" required>
                    <option v-for="item in masterDataStore.manufacturers" :key="item.id" :value="item.id">{{ item.name }}</option>
                  </select>
                </div>
                <TextArea name="notes" placeholder="Write notes for this manufacturer" class="outlined" label="Notes"></TextArea>
...
import * as yup from 'yup'
 ...
 const schema = yup.object({
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    newValues.manufacturerId = manufacturerId.value
    ...
    if (regButtonClick.value == 2) {
      ...
      await masterDataStore.createVariant(newValues)
